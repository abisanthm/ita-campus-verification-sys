                            ...
                            <div>
                                ...
                                <form action="{{ route('verify-certificate') }}" method="POST" class="py-3">
                                    ...
                                    <div class="form-group">
                                        <input type="text" name="search_id" class="form-control @error('search_id') is-invalid @enderror" placeholder="Enter {{ config('field_name.search_term') }}">
                                        @error('search_id')
                                            <span class="text-danger">{{ $errors->first('search_id') }}</span>
                                        @enderror
                                    </div>
                                    <div class="form-group">
                                        <button type="submit" class="btn btn-sm btn-primary btn-block">Verify</button>
                                        <button type="reset" class="btn btn-sm btn-danger btn-block">Clear</button>
                                    </div>
                                    <div class="">
                                        If you need any clarification, please contact us at <a href="mailto:{{ $setting->email }}">{{ $setting->email }}</a>
                                    </div>
                                    <div class="py-1">
                                        &copy; {{ date('Y') }} {{ $setting->company_name }}. All Rights Reserved | Powered By :<a href="https://www.inspirodesk.com/"> Inspiro Desk</a> 
                                    </div>
                                </form>
                            </div>
                        </div>
